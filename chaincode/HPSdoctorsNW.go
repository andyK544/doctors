...
func (self *DoctorsNWChainCode) Invoke(stub shim.ChaincodeStubInterface,
	...
	if function == "DeleteDoctor" {
		fmt.Println("invoking DeleteDoctor " + function)
		testBytes, err := DeleteDoctor(args[0], stub)
		if err != nil {
			fmt.Println("Error performing AddDoctor ")
			return nil, err
		}
		fmt.Println("Processed DeleteDoctor successfully. ")
		return testBytes, nil
	}
...
func RemoveDocFromSearchList(DocSpec SearchList, stub shim.ChaincodeStubInterface) error {
	// Checking if NPI_ID already present.

	var r []string
	s, err := GetDocList(DocSpec.SearchKeyWord, stub)
	fmt.Println("This is the list of NPI_IDs under SearchKey -->", DocSpec.SearchKeyWord)
	fmt.Println("This is searchlist for:", DocSpec.SearchKeyWord, "-->", s)
	length := len(s)
...
func AddDoctor(userJSON string, stub shim.ChaincodeStubInterface) ([]byte, error) {
	...
	// All to  AllDoctors Key
	SL.NPI_ID = res.NPI_ID
	SL.SearchKeyWord = `AllDoctors`
	fmt.Println("Printing Searchlist for Speciality -->", SL)
	testBytes, err1 = AddDocToSearchList(SL, stub)
	if err1 != nil {
		fmt.Println("Failed to add Doctor to Speciality search ")
...
func DeleteDoctor(DocSpec string, stub shim.ChaincodeStubInterface) ([]byte, error) {
	fmt.Printf("Running DeleteDoctor")
	var SL SearchList

	// Delete the key from the state in ledger
	err := stub.DelState(DocSpec)
	if err != nil {
		return nil, errors.New("Failed to DeleteDoctor state")
	}

	// Remove Doctor NPI_ID from master list.
	SL.NPI_ID = DocSpec
	SL.SearchKeyWord = `AllDoctors`
	err = RemoveDocFromSearchList(SL, stub)
